...
import { ACCOUNTS, ROLE_INFO, type Account } from '@/lib/users'

export default function SettingsView({ account }: { account: Account | null }) {
  return (
      ...
      <div className="px-4 sm:px-8 pt-8 sm:pt-10 pb-16 max-w-3xl">
        <div className="mb-8 sm:mb-10">
          ...
          <p className="mt-2 text-[13px] text-zinc-500 dark:text-zinc-200 leading-relaxed">
            Theme, wallpaper and menu are saved in this browser, not against your
            account — sign in somewhere else and you get the defaults there.
          </p>
        </div>
...
/**
 * The roster, read-only. There is no user table behind it — the two accounts
 * are a constant in `lib/users`, and their passwords are environment variables
 * — so there is nothing here to edit from a screen. Adding a third person is a
 * commit, and this section says so rather than offering a button that lies.
 */
function UsersSection({ account }: { account: Account | null }) {
  return (
    <Section
      icon={Users}
      accent="text-emerald-400"
      title="Users"
      description="Who can sign in, and how far each of them gets."
    >
      <ul className="space-y-2">
        {ACCOUNTS.map((user) => {
          const role = ROLE_INFO[user.role]
          const isYou = user.username === account?.username

          return (
            <li
              key={user.username}
              className="flex items-start gap-3 px-3 py-3 rounded-xl border border-zinc-200 dark:border-white/[0.07] panel bg-zinc-50 dark:bg-white/[0.03]"
            >
              <div className="shrink-0 w-8 h-8 rounded-full border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-white/[0.06] flex items-center justify-center text-[13px] font-semibold text-zinc-700 dark:text-white uppercase">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`px-1.5 py-0.5 rounded-md border text-[12px] font-medium ${role.badge}`}>
                    {role.label}
                  </span>
                  {isYou && (
                    <span className="flex items-center gap-1 px-1.5 py-0.5 rounded-md border border-zinc-200 dark:border-white/[0.08] text-[12px] font-medium text-zinc-500 dark:text-zinc-200">
                      <ShieldCheck size={11} />
                      Signed in
                    </span>
                  )}
                </div>
                <p className="mt-1 text-[13px] text-zinc-500 dark:text-zinc-200 leading-relaxed">
                  {role.summary}
                </p>
              </div>
            </li>
      ...
      <p className="mt-3 text-[13px] text-zinc-500 dark:text-zinc-200 leading-relaxed">
