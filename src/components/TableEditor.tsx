    ...
    <div className="min-h-screen flex flex-col px-4 sm:px-8 py-6 sm:py-10">
      ...
      <div className="panel rounded-2xl border border-zinc-200 dark:border-white/[0.07] px-4 py-3 mb-4">
        <div className="flex items-center gap-2 flex-wrap mb-2">
          <Link href="/tables" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-50 dark:bg-white/[0.03] text-zinc-600 dark:text-zinc-100 hover:text-zinc-900 dark:hover:text-white transition-all">
            <ChevronLeft size={13} />
            Tables
          </Link>

          <p className="text-[13px] text-zinc-500 dark:text-zinc-200 tabular-nums">{wordCount}</p>
          ...
          <button onClick={() => setShowImport((v) => !v)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-50 dark:bg-white/[0.03] text-zinc-600 dark:text-zinc-100 hover:text-zinc-900 dark:hover:text-white transition-all">
            <Clipboard size={12} /><span className="hidden sm:inline">Import</span>
          </button>
          <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-50 dark:bg-white/[0.03] text-zinc-600 dark:text-zinc-100 hover:text-zinc-900 dark:hover:text-white transition-all">
      ...
      {/* Import panel */}
      {showImport && (
        <div className="mb-6 relative overflow-hidden rounded-xl border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-50 dark:bg-white/[0.03] p-4 sm:p-5">
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            <p className="text-[13px] text-zinc-500 dark:text-zinc-200">This replaces current table contents.</p>
          </div>
        </div>
      )}

      {/* Spreadsheet grid */}
      <div ref={tableRef} className="panel flex-1 overflow-x-auto rounded-2xl border border-zinc-200 dark:border-white/[0.07]">
        <table className="min-w-full border-collapse">
          <thead>
            <tr className="panel bg-zinc-100/60 dark:bg-white/[0.04]">
              ...
              <th className="border-b border-zinc-200 dark:border-white/[0.06] w-10">
                <button onClick={addColumn}
                  className="w-full h-full flex items-center justify-center py-2 px-2 text-zinc-500 dark:text-zinc-200 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-white/[0.04] transition-all">
                  <Plus size={13} />
                </button>
              </th>
            </tr>
                ...
                {columns.map((col, colIdx) => (
                  <td key={col.id} className="border-b border-r border-zinc-200/60 dark:border-white/[0.04] p-0">
                    <input data-r={rowIdx} data-c={colIdx}
                      ...
                      className="w-full px-3 py-2 text-sm text-zinc-700 dark:text-zinc-100 bg-transparent outline-none focus:bg-indigo-500/[0.06] focus:text-zinc-900 dark:focus:text-white transition-colors"
                    />
                  </td>
                ))}
