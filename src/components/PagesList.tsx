...
import { movePageToFolder, deletePage, deleteFolder, duplicatePage } from '@/lib/actions'
...
export default function PagesList({ pages: initial, folders: initialFolders, folderId }: Props) {
  ...
  function handleDuplicatePage(e: React.MouseEvent, pageId: string) {
    e.preventDefault()
    e.stopPropagation()
    startTransition(async () => {
      ...
      <>
        ...
        {pages.length === 0 && folders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-zinc-200/60 dark:border-white/[0.06]">
            <p className="text-sm text-zinc-500 mb-1 dark:text-zinc-200">No pages yet</p>
            <p className="text-[13px] text-zinc-500 dark:text-zinc-200">Hit "New Page" or drag pages here from the root</p>
          </div>
        ) : pages.length === 0 ? null : (
          <div className="space-y-1.5">
            {pages.map((page, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {page.content ? (
                        <p className="text-[13px] text-zinc-500 dark:text-zinc-200 mt-0.5 truncate">
            ...
            {folders.map((folder) => (
              ...
              >
                ...
                >
                  ...
                  <FolderOpen
                    size={14}
                    className={`shrink-0 transition-colors ${dragOverFolderId === folder.id ? 'text-amber-400/80' : 'text-zinc-500 dark:text-zinc-200 group-hover:text-amber-400/70'}`}
                  />
