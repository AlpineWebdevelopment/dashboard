// Hungarian display helpers for the CRM. Shared by both screens so the worklist
...
/**
 * 'ma esedékes', '2 napja esedékes', '3 nap múlva'.
 *
 ...
 * wording uses calendar days, because 'ma esedékes' is what a person expects to
 * see all day.
 */
export function due(iso: string | null, now: Date = new Date()): Due {
  if (!iso) return { text: 'nincs dátum', overdue: false, none: true }

  const date = new Date(iso)
  if (Number.isNaN(date.getTime())) {
    return { text: 'nincs dátum', overdue: false, none: true }
  }

  const overdue = date.getTime() <= now.getTime()
  const days = dayDelta(date, now)

  if (days === 0) return { text: overdue ? 'ma esedékes' : 'ma', overdue, none: false }
  if (days === -1) return { text: 'tegnap óta esedékes', overdue, none: false }
  if (days === 1) return { text: 'holnap', overdue, none: false }
  if (days < 0) return { text: `${Math.abs(days)} napja esedékes`, overdue, none: false }
  return { text: `${days} nap múlva`, overdue, none: false }
}
