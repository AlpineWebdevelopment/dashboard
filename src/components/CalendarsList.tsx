...
export default function CalendarsList({ calendars: initial, folders, folderId }: Props) {
      ...
      <>
        {calendars.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-white/[0.06]">
            ...
            <p className="text-xs text-zinc-700">Hit "New Calendar" or drag one here</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <Link
                    ...
                    className={`group relative flex items-center justify-between px-5 py-4 rounded-xl border border-white/[0.05] bg-white/[0.02] ${hover} transition-all duration-200 overflow-hidden cursor-grab active:cursor-grabbing`}
                  >
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {cal.goal && (
                          <p className="text-xs text-zinc-600 mt-0.5 truncate">{cal.goal}</p>
                        )}
                      </div>
                    </div>
  ...
  return (
    <>
      {folders.length > 0 && (
        <div className="mb-6">
          <p className="text-[11px] font-medium tracking-widest uppercase text-zinc-700 mb-3">Folders</p>
          <div className="space-y-1.5">
            {folders.map((folder) => (
              <div
                key={folder.id}
              ...
              >
                ...
                >
                  ...
                  <FolderOpen size={14} className={`shrink-0 transition-colors ${dragOverFolderId === folder.id ? 'text-amber-400/80' : 'text-zinc-600 group-hover:text-amber-400/70'}`} />
      ...
      {folders.length > 0 && calendars.length > 0 && (
        <p className="text-[11px] font-medium tracking-widest uppercase text-zinc-700 mb-3">Calendars</p>
      )}
