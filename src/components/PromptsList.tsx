    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 sm:py-28 rounded-2xl border border-dashed border-zinc-200/60 dark:border-white/[0.06]">
        <div className="w-11 h-11 rounded-xl border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-100/60 dark:bg-white/[0.03] flex items-center justify-center mb-4">
          <Sparkles size={16} className="text-zinc-500 dark:text-zinc-200" />
        </div>
        <p className="text-sm text-zinc-500 mb-1 dark:text-zinc-200">No prompts yet</p>
        <p className="text-[13px] text-zinc-500 dark:text-zinc-200">Hit &quot;New Prompt&quot; to save your first one</p>
      </div>
    )
  }
      ...
      {prompts.map((prompt, i) => (
        <div key={prompt.id} className="group/row relative">
          <Link
            ...
            className="group relative flex items-start justify-between px-5 py-4 rounded-xl border border-zinc-200 dark:border-white/[0.05] panel bg-zinc-50/50 dark:bg-white/[0.02] hover:bg-zinc-100 dark:hover:bg-white/[0.05] hover:border-zinc-300 dark:hover:border-orange-500/20 transition-all duration-200 overflow-hidden"
          >
            ...
            <div className="flex items-start gap-4 min-w-0 flex-1 pr-28">
              <span className="text-[13px] text-zinc-500 dark:text-zinc-200 tabular-nums w-5 text-right shrink-0 mt-0.5">
              ...
              <div className="min-w-0">
                ...
                {prompt.content ? (
                  <p className="text-[13px] text-zinc-500 dark:text-zinc-200 mt-0.5 line-clamp-2 leading-relaxed">
                    {prompt.content}
                  </p>
                ) : (
                  <p className="text-[13px] text-zinc-500 dark:text-zinc-200 mt-0.5 italic">Empty</p>
                )}
              </div>
            </div>
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover/row:opacity-100 flex items-center gap-0.5 transition-opacity">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded-md text-[12px] font-medium text-zinc-500 dark:text-zinc-200 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-500/10 transition-all"
            >
              {copied === prompt.id ? (
