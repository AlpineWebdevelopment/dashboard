...
export default function ScratchPad({ initial }: { initial: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-50 dark:bg-white/[0.03]">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-2">
        ...
        <span className="flex items-center gap-1 text-[12px] text-zinc-500 dark:text-zinc-200 h-4">
          {status === 'saving' && (
      ...
      <textarea
        ...
        rows={4}
        className="w-full bg-transparent px-5 pb-5 pt-1 text-sm text-zinc-700 dark:text-zinc-100 placeholder-zinc-500 dark:placeholder-zinc-400 outline-none resize-none leading-relaxed"
      />
    </div>
  )
}
