...
export default function CalendarMiniCard({
  ...
  const colorText = COLOR_TEXT[calendar.color] ?? 'text-zinc-500 dark:text-zinc-200'
  ...
  return (
    <div
      className={`flex flex-col rounded-2xl border border-zinc-200 dark:border-white/[0.07] panel bg-zinc-50/50 dark:bg-white/[0.02] hover:bg-zinc-100/60 dark:hover:bg-white/[0.04] ${colorBorder} p-4 transition-all duration-200 group cursor-pointer`}
      onClick={() => router.push(`/calendars/${calendar.id}`)}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className={`w-8 h-8 rounded-xl border border-zinc-200 dark:border-white/[0.08] panel bg-zinc-50 dark:bg-white/[0.04] flex items-center justify-center shrink-0 ${iconColor}`}>
            {hasValidIcon
          ...
          <div className="min-w-0">
            <Link
              ...
              className="text-[13px] font-semibold text-zinc-800 dark:text-white hover:text-zinc-900 dark:hover:text-white transition-colors truncate block"
            >
              {calendar.name}
            </Link>
            {calendar.goal && (
              <p className="text-[12px] text-zinc-500 dark:text-zinc-200 truncate mt-0.5">{calendar.goal}</p>
            )}
          </div>
        </div>
        {streak > 0 && (
      ...
      <div className="flex items-center justify-between mb-2">
        <button
          ...
          className="p-0.5 text-zinc-500 dark:text-zinc-200 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors"
        >
          <ChevronLeft size={12} />
        </button>
        <span className="text-[12px] font-medium text-zinc-500 dark:text-zinc-200 tabular-nums tracking-wide">
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {['S','M','T','W','T','F','S'].map((d, i) => (
          <div key={i} className="aspect-square flex items-center justify-center">
            <span className="text-[11px] text-zinc-500 dark:text-zinc-200 font-medium">{d}</span>
          </div>
        ))}
      </div>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isFuture ? 'opacity-45 cursor-default' : 'hover:opacity-80 active:scale-90 transition-transform'}
                ${hasBg ? STATUS_BG[status] : 'panel bg-zinc-200/70 dark:bg-white/[0.05] hover:bg-zinc-200 dark:hover:bg-white/[0.09]'}
                ${isToday && !hasBg ? 'ring-1 ring-white/30' : ''}
              `}
            >
              <span className={`text-[11px] font-medium tabular-nums leading-none ${
                hasBg ? 'text-white/80' : isToday ? colorText : 'text-zinc-500 dark:text-zinc-200'
              }`}>
                {day}
              </span>
            </div>
      ...
      <div className="mt-auto space-y-2">
        <div className="flex items-center justify-between text-[12px]">
          <div className="flex items-center gap-2.5">
            {greenCount > 0  && <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" /><span className="text-zinc-500 dark:text-zinc-200">{greenCount}</span></span>}
            {yellowCount > 0 && <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block" /><span className="text-zinc-500 dark:text-zinc-200">{yellowCount}</span></span>}
            {redCount > 0    && <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-rose-500 inline-block" /><span className="text-zinc-500 dark:text-zinc-200">{redCount}</span></span>}
          </div>
          <span className={`font-semibold tabular-nums ${colorText}`}>{greenPct}%</span>
        </div>
        <div className="h-1 rounded-full panel bg-zinc-200 dark:bg-white/[0.06] overflow-hidden">
          <div
