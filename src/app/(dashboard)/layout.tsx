...
import { HOLIDAYS_COOKIE, NAV_COOKIE, NEWS_COOKIE } from "@/lib/prefs";
...
export default async function DashboardLayout({
  ...
  const [[navPref, showNews, showHolidays], account] = await Promise.all([
    cookies().then((c) => [
      c.get(NAV_COOKIE)?.value ?? null,
      c.get(NEWS_COOKIE)?.value !== "0",
      c.get(HOLIDAYS_COOKIE)?.value !== "0",
    ] as const),
  ...
  return (
    <SessionProvider account={account}>
      ...
      <NavPrefsProvider
        initial={navPref}
        initialShowNews={showNews}
        initialShowHolidays={showHolidays}
      >
