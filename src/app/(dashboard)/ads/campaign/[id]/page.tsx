    ...
    try {
      for (const a of metaAds.filter((a) => selected.has(a.id))) {
        await insertAd(campaignId, {
          ...
          metaAdsetId: selectedAdSet?.id ?? null,
          metaAdsetName: selectedAdSet?.name ?? null,
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-[rgba(14,14,22,0.98)] border border-violet-500/20 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-3 border-t border-white/[0.06] shrink-0">
          <p className="text-[11px] text-zinc-600">💡 After pasting, ask follow-ups like "what would you change about my offer?" or "write me a hook for the next test"</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-[rgba(14,14,22,0.98)] border border-white/[0.1] rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto flex-1">
            ...
            <div className="flex flex-wrap gap-1.5">
              {CONCEPTS.map((c) => (
                <button key={c} onClick={() => setConcept(c)}
                  className={`px-2.5 py-1.5 rounded-lg text-xs border transition-all ${concept === c ? "bg-indigo-600/30 border-indigo-500/50 text-indigo-300" : "border-white/[0.07] bg-white/[0.03] text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.06]"}`}>
                  {conceptEmojis[c]} {c}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {FORMATS.map((f) => (
                <button key={f} onClick={() => setFormat(f)}
                  className={`px-2.5 py-1.5 rounded-lg text-xs border transition-all ${format === f ? "bg-violet-600/30 border-violet-500/50 text-violet-300" : "border-white/[0.07] bg-white/[0.03] text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.06]"}`}>
                  {f}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {AWARENESS_TEST_LEVELS.map((a) => (
                <button key={a} onClick={() => setAwareness(a)}
                  className={`px-2.5 py-1.5 rounded-lg text-xs border transition-all ${awareness === a ? "bg-yellow-600/20 border-yellow-500/40 text-yellow-300" : "border-white/[0.07] bg-white/[0.03] text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.06]"}`}>
                  {a}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea value={massDesire} onChange={(e) => setMassDesire(e.target.value)}
              placeholder="What core desire, fear, or aspiration does this ad tap into?" rows={3}
          ...
          <div>
            ...
            <input value={pricingOffer} onChange={(e) => setPricingOffer(e.target.value)}
          ...
          <div>
            ...
            <textarea value={hook} onChange={(e) => setHook(e.target.value)}
              placeholder="Specific hook, opening line, or angle you're testing" rows={2}
          ...
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              placeholder="What did you observe or learn from this test?" rows={2}
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-white/[0.18] focus:bg-white/[0.06] transition-all resize-none" />
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {DURATION_OPTIONS.map((d) => (
                <button key={d} onClick={() => setDuration(d)}
                  className={`px-3 py-1.5 rounded-lg text-xs border transition-all ${duration === d ? "bg-indigo-600/30 border-indigo-500/50 text-indigo-300" : "border-white/[0.07] bg-white/[0.03] text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.06]"}`}>
                  {d}d
                </button>
              ))}
            </div>
...
function AdRow({
  ...
  ad: Ad;
  onStatusChange: (id: string, s: Ad["status"]) => void;
  onEdit: (ad: Ad) => void;
  ...
  return (
    <tr className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
      ...
      <td className="px-3 py-3 min-w-[160px] max-w-[260px]">
        ...
        {ad.metaAdsetName && (
          <p className="text-[10px] text-zinc-600 mt-0.5 truncate">{ad.metaAdsetName}</p>
        )}
      </td>
      ...
      {/* Format */}
      <td className="px-3 py-3 whitespace-nowrap">
        <span className="text-[11px] text-zinc-400">{ad.format}</span>
      </td>

      {/* Awareness */}
      <td className="px-3 py-3 whitespace-nowrap">
        <span className="text-[11px] text-yellow-400/80">{ad.awareness}</span>
      </td>

      {/* Test Focus */}
      <td className="px-3 py-3 whitespace-nowrap">
        <span className="text-[11px] text-zinc-500 capitalize">{ad.testFocus}</span>
      </td>
      ...
      <td className="pr-4 pl-2 py-3 whitespace-nowrap">
        <div className="flex items-center gap-0.5">
          ...
          <button onClick={() => onEdit(ad)} title="Edit"
            className="w-6 h-6 rounded text-[11px] border border-white/[0.05] bg-transparent text-zinc-600 hover:text-zinc-200 hover:border-white/[0.12] transition-colors ml-0.5">
            ✎
          </button>
          <button
            ...
            className={`w-6 h-6 rounded text-[10px] border transition-colors ${
              ...
              : "border-white/[0.05] bg-transparent text-zinc-700 hover:text-red-400 hover:border-red-500/20"
            }`}>
            {confirmDelete ? "?" : "✕"}
          </button>
        </div>
...
function AdSetRow({
  ...
  const avgCtr      = withIns.length > 0
    ? withIns.reduce((s, a) => s + parseFloat(a.metaInsights!.ctr || "0"), 0) / withIns.length
    : null;
  const avgCpc      = withIns.length > 0 && withIns.some((a) => a.metaInsights?.costPerClick)
    ? withIns.reduce((s, a) => s + parseFloat(a.metaInsights?.costPerClick || "0"), 0) / withIns.filter((a) => a.metaInsights?.costPerClick).length
    : null;
  ...
  return (
    <tr className="border-b border-white/[0.04] hover:bg-white/[0.02] cursor-pointer transition-colors group" onClick={onClick}>
      ...
      <td className="px-3 py-3.5 text-right">
        <span className="text-[12px] text-zinc-400">{ads.length}</span>
      </td>
      <td className="px-3 py-3.5">
        <div className="flex items-center gap-2 text-[11px]">
          {testing > 0 && <span className="text-indigo-400">{testing} testing</span>}
          {winners > 0 && <span className="text-emerald-400">{winners} winner{winners !== 1 ? "s" : ""}</span>}
          {losers  > 0 && <span className="text-red-400">{losers} loser{losers !== 1 ? "s" : ""}</span>}
        </div>
      </td>
      ...
      <td className="px-3 py-3.5 text-right text-[12px] text-zinc-300 tabular-nums pr-4">{spend > 0 ? `$${spend.toFixed(2)}` : <span className="text-zinc-700">—</span>}</td>
...
export default function CampaignPage() {
  ...
  const syncMeta = async (overrideCampaign?: Campaign, overridePreset?: DatePreset) => {
    ...
    try {
      ...
      const insights = await metaApi("syncInsights", { adIds, datePreset: preset });
      await Promise.all(
  ...
  // Initial load + auto-sync if ads have metaAdId but no insights
  useEffect(() => {
    load().then((c) => {
      if (!c) return;
  ...
  const handleDatePreset = async (preset: DatePreset) => {
    setDatePreset(preset);
    await syncMeta(undefined, preset);
  };
  ...
  const adSets = useMemo(() => {
    ...
    for (const ad of allAds) {
      ...
      if (!map.has(ad.metaAdsetId)) {
        map.set(ad.metaAdsetId, { id: ad.metaAdsetId, name: ad.metaAdsetName ?? ad.metaAdsetId, ads: [] });
      }
  ...
  if (error) return (
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/ads" className="text-indigo-400 text-sm hover:underline">← Back</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      {/* ─── Header ── */}
      <header className="sticky top-0 z-40 border-b border-white/[0.05] bg-[rgba(7,7,15,0.9)] backdrop-blur-xl">
        <div className="max-w-[1400px] mx-auto px-5">
          ...
          <div className="flex items-center justify-between gap-4 pt-3.5 pb-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button onClick={() => setShowImport(true)}
                className="px-3 py-1.5 rounded-lg text-xs border border-sky-500/20 bg-white/[0.03] text-zinc-300 hover:text-white hover:bg-white/[0.06] transition-all">
                📥 Import
              </button>
              {allAds.length > 0 && (
                <button onClick={() => setShowAnalyze(true)}
                  className="px-3 py-1.5 rounded-lg text-xs border border-violet-500/20 bg-white/[0.03] text-zinc-300 hover:text-white hover:bg-white/[0.06] transition-all">
                  🤖 Analyze
                </button>
              )}
              <button onClick={openNew}
                className="px-3.5 py-1.5 rounded-lg bg-indigo-600 text-sm font-medium hover:bg-indigo-500 transition-colors text-white">
                + New Test
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 pb-0">
            <button
              ...
              Ad Sets
              {adSets.length > 0 && (
                <span className="ml-1.5 text-[10px] text-zinc-600">{adSets.length}</span>
              )}
            </button>
            <button
              ...
              Ads
              {allAds.length > 0 && (
                <span className="ml-1.5 text-[10px] text-zinc-600">{allAds.length}</span>
              )}
            </button>
          </div>
        </div>
